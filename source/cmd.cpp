...
// copy off any commands still remaining in the exec buffer
	...
	if (templen)
	{
		temp = (char*)qCZone::Alloc (templen);
...
char *CopyString (char *in)
{
	...
	out = (char*)qCZone::Alloc (strlen(in)+1);
	strcpy (out, in);
	return out;
}
...
void Cmd_Alias_f (void)
{
	...
	if (!a)
	{
		a = (cmdalias_t*)qCZone::Alloc (sizeof(cmdalias_t));
		a->next = cmd_alias;
		cmd_alias = a;
	}
	strcpy (a->name, s);
...
// skip whitespace up to a /n
		...
		{
			cmd_argv[cmd_argc] = (char*)qCZone::Alloc (strlen(com_token)+1);
...
// fail if the command is a variable name
	if (qCCVar::GetString(cmd_name)[0])
	{
...
// fail if the command already exists
	...
	cmd = (cmd_function_t*)qCHunk::Alloc (sizeof(cmd_function_t));
	cmd->name = cmd_name;
