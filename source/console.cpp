...
qSCVar		con_notifytime = {"con_notifytime","3"};		//seconds
...
{
	qCInput::SetKeyDestination( qMESSAGE );
	team_message = FALSE;
}
...
#define MAXGAMEDIRLEN	1000
	...
	con_text = (char*)qCHunk::AllocName (CON_TEXTSIZE, "context");
	memset (con_text, ' ', CON_TEXTSIZE);
	con_linewidth = -1;
	Con_CheckResize ();
...
//
	qCCVar::Register (&con_notifytime);
...
void Con_DrawInput (void)
{
	...
	if (qCInput::GetKeyDestination() != qCONSOLE && !con_forcedup)
...
void Con_DrawNotify (void)
{
	...
	if (qCInput::GetKeyDestination() == qMESSAGE )
	{
		x = 0;
...
void Key_Console (int key)
{
	...
	if (key == K_TAB)
	{
		...
		v = qCCVar::CountMatches(key_lines[edit_line]+1);
		...
		if (c + v + a > 1)	// More than a single possible match
		{
			...
			if (c)
			{
				if (c==1)
					...
					Con_Printf("%d possible commands:\n", c);
			...
			if (v)
			{
				if (v==1)
					...
					Con_Printf("%d possible variables:\n", v);

				qCCVar::PrintMatches(key_lines[edit_line]+1);
			}
		...
		if (v)
			cmd = qCCVar::CompleteVariable(key_lines[edit_line]+1);
	...
	if (key == K_RIGHTARROW)
	{
		if (strlen(key_lines[edit_line]) == (unsigned)key_linepos)
		{
