...
typedef struct mplane_s
{
	qVector3	normal;
	float	dist;
	qUInt8	type;			// for texture axis selection and fast side tests
...
// lighting info
	...
	qUInt8		*samples;		// [numstyles*surfsize]
} msurface_t;
...
typedef struct {
	...
	int			version;
	qVector3		scale;
	qVector3		scale_origin;
	float		boundingradius;
	qVector3		eyeposition;
	int			numskins;
	int			skinwidth;
	int			skinheight;
	int			numverts;
...
//
	...
	qUInt32		numleafs;		// number of visible leafs, not counting 0
	mleaf_t		*leafs;
...
//
	qSCache	cache;		// only access through Mod_Extradata
